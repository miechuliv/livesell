...
class ModelSaleOrder extends Model {
	public function addOrder($data, $ebay = false, $allegro = false) {
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
                ...
                'auction'   => $order_query->row['auction'],
	...
	public function addOrderHistory($order_id, $data) {
        ...
        if($data['send_sms']){
            $msg = $data['sms_message'];
            $msg = str_ireplace('{STATUS}',$order_status_query->row['name'],$msg);
            $msg = str_ireplace('{ID}',$order_id,$msg);

            OpenCartSms::sendSms($this->config->get('config_smsapi_number'),$msg,$this->config->get('config_smsapi_sender'));
        }
        // edn send sms
